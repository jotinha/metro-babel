...
doubleLoop funacc funpair x:xs = funacc (singleLoop funacc (funpair x) xs)) (doubleLoop funpair xs)
doubleLoop funacc funpair [] = 0

singleLoop funacc funone x:xs = funacc x (singleLoop xs)
...
enerTotal pos box rcsq = doubleLoop (+) enerPair' pos
  where enerPair' p0 p1 = enerPair p0 p1 box rcsq

enerOnePos p1 posrest box rcsq = singleLoop (+) enerPair' posrest
  where enerPair' p0 = enerPair p0 p1 box rcsq
